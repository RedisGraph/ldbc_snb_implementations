...
for run in $(seq ${REPETITIONS}); do
  ...
  RESULTS_DIR="results/SF_0.1_${CLIENTS}CLIENTS_results/${PREFIX}run_${run}"
  redis-cli -h ${DATABASE_HOST} -p ${DATABASE_PORT} config resetstat

  OUT_FULL_FILE_NAME="${PREFIX}SF_0.1_${CLIENTS}CLIENTS_result_run_${run}.out"
  echo "Saving results to ${OUT_FULL_FILE_NAME}"
  mkdir -p ~/${RESULTS_DIR}

  java -classpath "target/redisgraph-0.4.0-SNAPSHOT.jar" com.ldbc.driver.Client -P interactive-benchmark-SF0.1.properties \
